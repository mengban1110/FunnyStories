...
public class EmailUtils {
    ...
    static{
    	try {
    		//读取配置文件
			Map mail = PropertyUtils.getyInfoOfPropert("/mail.properties");
	...
	public static void createMimeMessage(String receiveMail,String code,Date date) throws UnsupportedEncodingException, MessagingException{
		...
		Session session = getSession();
	  	...
	  	//4.发件人
	  	message.setFrom(new InternetAddress(sendEmailAccount, "糗事百科 ", "UTF-8"));
	   
	  	//5.收件人
	  	message.setRecipient(RecipientType.TO, new InternetAddress(receiveMail, "接受者：用户", "UTF-8"));
	  	//message.setRecipient(RecipientType.CC, new InternetAddress(sendEmailAccount, "抄送给自己的", "utf-8"));
	  	//6.设定主题
	  	message.setSubject("测试系统验证码","UTF-8");
	   
	  	//7.设定正文
	  	message.setContent("【糗事百科】你正在使用该邮箱进行注册，您的验证码为： "+code+"  ，若非本人操作，请忽视此邮件。", "text/html;charset=utf-8");
