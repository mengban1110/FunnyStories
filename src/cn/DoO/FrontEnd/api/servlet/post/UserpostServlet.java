...
public class UserpostServlet {
	...
	public void sendPost(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			// 判断是否没输入
			if ((text == null || text.equals("")) && (videoItem == null || videoItem.equals("")) && imageItems.size() < 1) {
				jsonObject = new JSONObject();
				jsonObject.put("code", "-1");
			...
			if (user.get("userstatus").toString().equals("3") || user.get("userstatus").toString().equals("5")) {
				jsonObject = new JSONObject();
				jsonObject.put("code", "-1");
			...
			if(imageItems != null || videoItem != null){
				...
				SendPostThread sendPostThread = 
						new SendPostThread(video, videoItem, videoPrifix, image, imageItems, prifixList, uid, text,  postDao);
